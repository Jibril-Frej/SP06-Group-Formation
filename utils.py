...
def load_data(label_file, translation_file):
    ...
    df_translated.insert(
        ...
        "treatment",
        [random.randint(0, 1) for i in range(len(df_translated))],
    )
...
def get_labels(m_interests, metric="euclidean", gamma=0.01):
    ...
    labels = SpectralClustering(
        n_clusters=2, random_state=123, affinity="precomputed"
    ).fit_predict(similarity)
    return labels


def hierarchical_spectral(
    m_matrix, labels, metric="euclidean", gamma=0.01, offset=2, verbose=0
):
    ...
    while len(labels_to_explore) >= 1:
        ...
        if size >= 4:
            ...
            new_unique_labels, new_counts_labels = np.unique(
                new_labels, return_counts=True
            )
            if np.min(new_counts_labels) >= 2:
                ...
                if verbose:
                    print(
                        f"Adding labels and removes current {i}. Updated list: {labels_to_explore}"
                    )
                unique_labels, counts_labels = np.unique(labels, return_counts=True)
                if verbose:
                ...
                if verbose:
                    print(
                        f"We do not accept the new labels. Counts {new_counts_labels}"
                    )
...
def get_contrasting_clusters(
    m_comp_strenghts, m_comp_interest, metric="euclidean", gamma=0.01, verbose=0
):
    distance_matrix = get_distance_matrix(
        m_comp_strenghts, m_comp_interest, verbose=verbose
    )
    ...
    new_labels = hierarchical_spectral_comp(
        m_comp_strenghts,
        m_comp_interest,
        labels,
        metric="euclidean",
...
def hierarchical_spectral_comp(
    m_comp_strenghts,
    m_comp_interest,
    labels,
    metric="euclidean",
    ...
    while len(labels_to_explore) >= 1:
        ...
        if size >= 4:
            ...
            new_labels = (
                get_labels_comp(m_aux_strenghts, m_aux_interest, metric, gamma) + offset
            )
            new_unique_labels, new_counts_labels = np.unique(
                new_labels, return_counts=True
            )
...
def get_group_sizes(n):
    if n % 4 == 0:
        ...
        group_sizes = [4] * (n // 4 - 1) + [5]
    elif n % 4 == 3:
        group_sizes = [4] * (n // 4) + [3]
    elif n % 4 == 2:
        group_sizes = [4] * (n // 4 - 1) + [3, 3]
    return group_sizes
...
def set_group_size(df, verbose=0):
    ...
    for value in list(
        ...
        for i, elem in enumerate(study_group_list):
            if elem == value:
                if counter == group_sizes[0]:
                    counter = 0
                    group_sizes.pop(0)
                    group += 1
                counter += 1
                study_group_list[i] = str(study_group_list[i]) + "_" + str(group)
    df.loc[:, "study_group"] = study_group_list


def group_by_similarity(
    df_translated, similar_interest_file, availabilities_file, verbose=0
):
    ...
    df_similar_available = df_similar_available.replace("Unchecked", 0).replace(
        "Checked", 1
    )
    m_available = np.array(df_similar_available)

    available_labels = hierarchical_spectral(
        m_available, labels, metric="hamming", gamma=0.01, offset=10, verbose=verbose
    )
...
def group_by_complementarity(
    ...
    labels = get_contrasting_clusters(
        m_comp_strength, m_comp_interest, metric="euclidean", gamma=0.01
    )
...
def get_groups(df_similar, df_comp):
    df_groups = pd.concat(
        [
            df_comp[["treatment", "study_group", "Record ID"]],
            df_similar[["treatment", "study_group", "Record ID"]],
        ],
        axis=0,
    )
    df_groups["treatment"] = (
        df_groups["treatment"].replace(0, "complementary").replace(1, "similar")
    )
    return df_groups
